...
function performAnalysis(source) {
      ...
      if (call.arguments.length === 1) {
        ...
        if (specifier.type === "Literal") {
          ...
          all(
            ...
            () => {
              exportsAllFrom.add(specifierString);
            }
          );
        }
      }
  ...
  const esModuleFlag = interopAssignments.every(({ computed, value }) => !computed && Boolean(value));
  return {
    staticAssignmentNames: staticAssignmentNames.keys(),
    exportsAllFrom,
    esModuleFlag
  };
};
